 ...
 */
public class Battle1ActivityOnline extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ref2 = ref1.child("Games").child(Integer.toString(gameRoomNumGlobal));

        //Putting Spaztic Random Delay To Help Ensure A Host is Made Properly
        final Handler handler = new Handler();

        final Random rand = new Random();
        final int b = rand.nextInt(1000);
                            ...
                            public void run() {
                                ref2.addListenerForSingleValueEvent(new ValueEventListener() {
                                    ...
                                    public void onDataChange(DataSnapshot dataSnapshot) {
                                        //Make a host if we haven't already done so
                                        if (!madeAHost)
                                        {
                                            if (dataSnapshot.child("Host").exists())
                                            {
                                                ...
                                                {
                                                    //we have already made a host, so we are not the host
                                                    host = false;
                                                    ref2.child("Client").setValue(name1);
                                                }

                                            }
                                            else
                                            {
                                                //Since it didnt work, there was nothing there, so we should put something there,
                                                //and make us the host
                                                ref2.child("Host").setValue(name1);
                                                host = true;
                                            }

                                            name2 = dataSnapshot.child("Users").child(otherPlayersKey).child("Name").getValue().toString();
                                        }

                                        int p2ActivePokemonDexNum =  Integer.parseInt(dataSnapshot.child("Users").child(otherPlayersKey).child("Active Pokemon").getValue().toString());

                                        if(!madeAHost)
                                        {
                                            madeAHost = true;

                                            //Get P2's full team from database
                                            for (int a = 0; a < 6; a++)
                                            {
                                                int p2PokemonDexNumber = Integer.parseInt(dataSnapshot.child("Users").child(otherPlayersKey).child("Pokemon").child(Integer.toString(a)).getValue().toString());

                                                /*if (host)
                                                    ref2.child("Reading").push().child("I am host and I read p2PokemonDexNumber in as ").setValue(p2PokemonDexNumber);
                                                else
                                                    ref2.child("Reading").push().child("I am client and I read p2PokemonDexNumber in as ").setValue(p2PokemonDexNumber);
                                                                                */
                                                for (int i = 0; i < 99; i++)
                                                {
                                                    if (p2PokemonDexNumber == pokemon[i].dexNumber)
                                                    {
                                                        p2Pokemon[a] = pokemon[i];
                                                        p2Pokemon[a].orderNum = a;
                                                        /*
                                                        if (!host)
                                                            ref2.child("Assigning").push().child("Assigned " + pokemon[i].name + " to slot " + a + " on " + name2 + "'s team and I am client").setValue("true");
                                                        else
                                                            ref2.child("Assigning").push().child("Assigned " + pokemon[i].name + " to slot " + a + " on " + name2 + "'s team and I am host").setValue("true");
                                                    */
                                                    }
                                                }
                                            }
                                        ...
                                        /*if(host)
                                            ref2.push().child("I am host and I read p2ActivePokemonDexNumber in as ").setValue(p2ActivePokemonDexNum);
                                        else
                                            ref2.push().child("I am client and I read p2ActivePokemonDexNumber in as ").setValue(p2ActivePokemonDexNum);
                                                            */
                                        for(int a=0;a<6;a++)
                                        {
                                            //Get P2's active Pokemon from their team
                                            if (p2ActivePokemonDexNum == p2Pokemon[a].dexNumber)
                                            {
                                                activePokemon2 = p2Pokemon[a];
                                                activePokemon2.orderNum=a;

                                                /*
                                                if(!host)
                                                    ref2.child("Assigning").push().child("After assigning " + activePokemon2.name + " to " + name2 + "'s team and I am client").setValue("true");
                                                else
                                                    ref2.child("Assigning").push().child("After assigning " + activePokemon2.name + " to " + name2 + "'s team and I am host").setValue("true");
                                                  */
                                            }
                                        }
                                        ...
                                        /*
                                        //Debugging purposes
                                        ref2.child("Users").child(nameKeyOurs).child("My Team").child("I Claim my active pokemon is " + activePokemon1.name).setValue("true");
                                        ref2.child("Users").child(otherPlayersKey).child("My Team").child("I Claim their active pokemon is " + activePokemon2.name).setValue("true");

                                        ref2.child("Users").child(nameKeyOurs).child("My Team").child("I Claim my first pokemon is " + p1Pokemon[0].name).setValue("true");
                                        ref2.child("Users").child(nameKeyOurs).child("My Team").child("I Claim my second pokemon is " + p1Pokemon[1].name).setValue("true");
                                        ref2.child("Users").child(nameKeyOurs).child("My Team").child("I Claim my third pokemon is " + p1Pokemon[2].name).setValue("true");
                                        ref2.child("Users").child(nameKeyOurs).child("My Team").child("I Claim my fourth pokemon is " + p1Pokemon[3].name).setValue("true");
                                        ref2.child("Users").child(nameKeyOurs).child("My Team").child("I Claim my fith pokemon is " + p1Pokemon[4].name).setValue("true");
                                        ref2.child("Users").child(nameKeyOurs).child("My Team").child("I Claim my sixth pokemon is " + p1Pokemon[5].name).setValue("true");

                                        ref2.child("Users").child(otherPlayersKey).child("My Team").child("I Claim their first pokemon is " + p2Pokemon[0].name).setValue("true");
                                        ref2.child("Users").child(otherPlayersKey).child("My Team").child("I Claim their second pokemon is " + p2Pokemon[1].name).setValue("true");
                                        ref2.child("Users").child(otherPlayersKey).child("My Team").child("I Claim their third pokemon is " + p2Pokemon[2].name).setValue("true");
                                        ref2.child("Users").child(otherPlayersKey).child("My Team").child("I Claim their fourth pokemon is " + p2Pokemon[3].name).setValue("true");
                                        ref2.child("Users").child(otherPlayersKey).child("My Team").child("I Claim their fith pokemon is " + p2Pokemon[4].name).setValue("true");
                                        ref2.child("Users").child(otherPlayersKey).child("My Team").child("I Claim their sixth pokemon is " + p2Pokemon[5].name).setValue("true");
                                        */
                                        ...
                                        //This changes after it reads from the database
                                        ...
                                        if (host)
                                        {
                                            ...
                                            myRef.child("Games").child(Integer.toString(gameRoomNumGlobal)).child("Users").child(nameKeyOurs).child("Battle Info").child("Absolute Start").setValue(absoluteStart1);
                                            myRef.child("Games").child(Integer.toString(gameRoomNumGlobal)).child("Users").child(otherPlayersKey).child("Battle Info").child("Absolute Start").setValue(absoluteStart2);
                                        ...
                                        //helpingDebugByUploadAfterCheckingDatabaseOnBattleActivityOnline();
                                    }
        ...
        //helpingDebugByUploadBeforeCheckingDatabaseOnBattleActivityOnline();

    }

    void helpingDebugByUploadAfterCheckingDatabaseOnBattleActivityOnline()
    {
        if(!host)
        {
            //add Firebase Database stuff
            ...
            myRef.child("Games").child("Important Info After Checking Database on BattleActivity").child("After done everything, client claims p1 flinch is").setValue(activePokemon1.flinch);
            myRef.child("Games").child("Important Info After Checking Database on BattleActivity").child("After done everything, client claims p2 flinch is").setValue(activePokemon2.flinch);
            myRef.child("Games").child("Important Info After Checking Database on BattleActivity").child("After done everything, client claims p1 fainted is").setValue(activePokemon1.fainted);
            myRef.child("Games").child("Important Info After Checking Database on BattleActivity").child("After done everything, client claims p2 fainted is").setValue(activePokemon2.fainted);
            myRef.child("Games").child("Important Info After Checking Database on BattleActivity").child("After done everything, client claims p1 damage is").setValue(p1Damage);
            myRef.child("Games").child("Important Info After Checking Database on BattleActivity").child("After done everything, client claims p2 damage is").setValue(p2Damage);
            myRef.child("Games").child("Important Info After Checking Database on BattleActivity").child("After done everything, client claims p1 crit is").setValue(p1Critt);
            myRef.child("Games").child("Important Info After Checking Database on BattleActivity").child("After done everything, client claims p2 crit is").setValue(p2Critt);
            myRef.child("Games").child("Important Info After Checking Database on BattleActivity").child("After done everything, client claims p1 hit is").setValue(activePokemon1.hit);
            myRef.child("Games").child("Important Info After Checking Database on BattleActivity").child("After done everything, client claims p2 hit is").setValue(activePokemon2.hit);
            myRef.child("Games").child("Important Info After Checking Database on BattleActivity").child("After done everything, client claims p1 fail is").setValue(usedMove1.fail);
            myRef.child("Games").child("Important Info After Checking Database on BattleActivity").child("After done everything, client claims p2 fail is").setValue(usedMove2.fail);
            myRef.child("Games").child("Important Info After Checking Database on BattleActivity").child("After done everything, client claims p1 move is").setValue(usedMove1.name);
            myRef.child("Games").child("Important Info After Checking Database on BattleActivity").child("After done everything, client claims p2 move is").setValue(usedMove2.name);
            myRef.child("Games").child("Important Info After Checking Database on BattleActivity").child("After done everything, client claims p1 switch is").setValue(p1Switch);
            myRef.child("Games").child("Important Info After Checking Database on BattleActivity").child("After done everything, client claims p2 switch is").setValue(p2Switch);
            myRef.child("Games").child("Important Info After Checking Database on BattleActivity").child("After done everything, client claims p1 hp is").setValue(activePokemon1.hp);
            myRef.child("Games").child("Important Info After Checking Database on BattleActivity").child("After done everything, client claims p2 hp is").setValue(activePokemon2.hp);
            myRef.child("Games").child("Important Info After Checking Database on BattleActivity").child("After done everything, client claims start1 is").setValue(start1);
            myRef.child("Games").child("Important Info After Checking Database on BattleActivity").child("After done everything, client claims start2 is").setValue(start2);
            myRef.child("Games").child("Important Info After Checking Database on BattleActivity").child("After done everything, client claims absoluteStart1 is").setValue(absoluteStart1);
            myRef.child("Games").child("Important Info After Checking Database on BattleActivity").child("After done everything, client claims absoluteStart2 is").setValue(absoluteStart2);
            myRef.child("Games").child("Important Info After Checking Database on BattleActivity").child("After done everything, client claims healthAfterHit1 is").setValue(healthAfterHit1);
            myRef.child("Games").child("Important Info After Checking Database on BattleActivity").child("After done everything, client claims healthAfterHit2 is").setValue(healthAfterHit2);
            myRef.child("Games").child("Important Info After Checking Database on BattleActivity").child("After done everything, client claims healthAfterHealing1 is").setValue(healthAfterHealing1);
            myRef.child("Games").child("Important Info After Checking Database on BattleActivity").child("After done everything, client claims healthAfterHealing2 is").setValue(healthAfterHealing2);
        }
        if(host)
        {
            //add Firebase Database stuff
            ...
            myRef.child("Games").child("Important Info After Checking Database on BattleActivity").child("After done everything, host claims p1 flinch is").setValue(activePokemon1.flinch);
            myRef.child("Games").child("Important Info After Checking Database on BattleActivity").child("After done everything, host claims p2 flinch is").setValue(activePokemon2.flinch);
            myRef.child("Games").child("Important Info After Checking Database on BattleActivity").child("After done everything, host claims p1 fainted is").setValue(activePokemon1.fainted);
            myRef.child("Games").child("Important Info After Checking Database on BattleActivity").child("After done everything, host claims p2 fainted is").setValue(activePokemon2.fainted);
            myRef.child("Games").child("Important Info After Checking Database on BattleActivity").child("After done everything, host claims p1 damage is").setValue(p1Damage);
            myRef.child("Games").child("Important Info After Checking Database on BattleActivity").child("After done everything, host claims p2 damage is").setValue(p2Damage);
            myRef.child("Games").child("Important Info After Checking Database on BattleActivity").child("After done everything, host claims p1 crit is").setValue(p1Critt);
            myRef.child("Games").child("Important Info After Checking Database on BattleActivity").child("After done everything, host claims p2 crit is").setValue(p2Critt);
            myRef.child("Games").child("Important Info After Checking Database on BattleActivity").child("After done everything, host claims p1 hit is").setValue(activePokemon1.hit);
            myRef.child("Games").child("Important Info After Checking Database on BattleActivity").child("After done everything, host claims p2 hit is").setValue(activePokemon2.hit);
            myRef.child("Games").child("Important Info After Checking Database on BattleActivity").child("After done everything, host claims p1 fail is").setValue(usedMove1.fail);
            myRef.child("Games").child("Important Info After Checking Database on BattleActivity").child("After done everything, host claims p2 fail is").setValue(usedMove2.fail);
            myRef.child("Games").child("Important Info After Checking Database on BattleActivity").child("After done everything, host claims p1 move is").setValue(usedMove1.name);
            myRef.child("Games").child("Important Info After Checking Database on BattleActivity").child("After done everything, host claims p2 move is").setValue(usedMove2.name);
            myRef.child("Games").child("Important Info After Checking Database on BattleActivity").child("After done everything, host claims p1 switch is").setValue(p1Switch);
            myRef.child("Games").child("Important Info After Checking Database on BattleActivity").child("After done everything, host claims p2 switch is").setValue(p2Switch);
            myRef.child("Games").child("Important Info After Checking Database on BattleActivity").child("After done everything, host claims p1 hp is").setValue(activePokemon1.hp);
            myRef.child("Games").child("Important Info After Checking Database on BattleActivity").child("After done everything, host claims p2 hp is").setValue(activePokemon2.hp);
            myRef.child("Games").child("Important Info After Checking Database on BattleActivity").child("After done everything, host claims start1 is").setValue(start1);
            myRef.child("Games").child("Important Info After Checking Database on BattleActivity").child("After done everything, host claims start2 is").setValue(start2);
            myRef.child("Games").child("Important Info After Checking Database on BattleActivity").child("After done everything, host claims absoluteStart1 is").setValue(absoluteStart1);
            myRef.child("Games").child("Important Info After Checking Database on BattleActivity").child("After done everything, host claims absoluteStart2 is").setValue(absoluteStart2);
            myRef.child("Games").child("Important Info After Checking Database on BattleActivity").child("After done everything, host claims healthAfterHit1 is").setValue(healthAfterHit1);
            myRef.child("Games").child("Important Info After Checking Database on BattleActivity").child("After done everything, host claims healthAfterHit2 is").setValue(healthAfterHit2);
            myRef.child("Games").child("Important Info After Checking Database on BattleActivity").child("After done everything, host claims healthAfterHealing1 is").setValue(healthAfterHealing1);
            myRef.child("Games").child("Important Info After Checking Database on BattleActivity").child("After done everything, host claims healthAfterHealing2 is").setValue(healthAfterHealing2);
        }
    }

    void helpingDebugByUploadBeforeCheckingDatabaseOnBattleActivityOnline()
    {
        if(!host)
        {
            //add Firebase Database stuff
            ...
            myRef.child("Games").child("Important Info Before Checking Database on BattleActivity").child("After done everything, client claims p1 flinch is").setValue(activePokemon1.flinch);
            myRef.child("Games").child("Important Info Before Checking Database on BattleActivity").child("After done everything, client claims p2 flinch is").setValue(activePokemon2.flinch);
            myRef.child("Games").child("Important Info Before Checking Database on BattleActivity").child("After done everything, client claims p1 fainted is").setValue(activePokemon1.fainted);
            myRef.child("Games").child("Important Info Before Checking Database on BattleActivity").child("After done everything, client claims p2 fainted is").setValue(activePokemon2.fainted);
            myRef.child("Games").child("Important Info Before Checking Database on BattleActivity").child("After done everything, client claims p1 damage is").setValue(p1Damage);
            myRef.child("Games").child("Important Info Before Checking Database on BattleActivity").child("After done everything, client claims p2 damage is").setValue(p2Damage);
            myRef.child("Games").child("Important Info Before Checking Database on BattleActivity").child("After done everything, client claims p1 crit is").setValue(p1Critt);
            myRef.child("Games").child("Important Info Before Checking Database on BattleActivity").child("After done everything, client claims p2 crit is").setValue(p2Critt);
            myRef.child("Games").child("Important Info Before Checking Database on BattleActivity").child("After done everything, client claims p1 hit is").setValue(activePokemon1.hit);
            myRef.child("Games").child("Important Info Before Checking Database on BattleActivity").child("After done everything, client claims p2 hit is").setValue(activePokemon2.hit);
            myRef.child("Games").child("Important Info Before Checking Database on BattleActivity").child("After done everything, client claims p1 fail is").setValue(usedMove1.fail);
            myRef.child("Games").child("Important Info Before Checking Database on BattleActivity").child("After done everything, client claims p2 fail is").setValue(usedMove2.fail);
            myRef.child("Games").child("Important Info Before Checking Database on BattleActivity").child("After done everything, client claims p1 move is").setValue(usedMove1.name);
            myRef.child("Games").child("Important Info Before Checking Database on BattleActivity").child("After done everything, client claims p2 move is").setValue(usedMove2.name);
            myRef.child("Games").child("Important Info Before Checking Database on BattleActivity").child("After done everything, client claims p1 switch is").setValue(p1Switch);
            myRef.child("Games").child("Important Info Before Checking Database on BattleActivity").child("After done everything, client claims p2 switch is").setValue(p2Switch);
            myRef.child("Games").child("Important Info Before Checking Database on BattleActivity").child("After done everything, client claims p1 hp is").setValue(activePokemon1.hp);
            myRef.child("Games").child("Important Info Before Checking Database on BattleActivity").child("After done everything, client claims p2 hp is").setValue(activePokemon2.hp);
            myRef.child("Games").child("Important Info Before Checking Database on BattleActivity").child("After done everything, client claims start1 is").setValue(start1);
            myRef.child("Games").child("Important Info Before Checking Database on BattleActivity").child("After done everything, client claims start2 is").setValue(start2);
            myRef.child("Games").child("Important Info Before Checking Database on BattleActivity").child("After done everything, client claims absoluteStart1 is").setValue(absoluteStart1);
            myRef.child("Games").child("Important Info Before Checking Database on BattleActivity").child("After done everything, client claims absoluteStart2 is").setValue(absoluteStart2);
            myRef.child("Games").child("Important Info Before Checking Database on BattleActivity").child("After done everything, client claims healthAfterHit1 is").setValue(healthAfterHit1);
            myRef.child("Games").child("Important Info Before Checking Database on BattleActivity").child("After done everything, client claims healthAfterHit2 is").setValue(healthAfterHit2);
            myRef.child("Games").child("Important Info Before Checking Database on BattleActivity").child("After done everything, client claims healthAfterHealing1 is").setValue(healthAfterHealing1);
            myRef.child("Games").child("Important Info Before Checking Database on BattleActivity").child("After done everything, client claims healthAfterHealing2 is").setValue(healthAfterHealing2);
        }
        if(host)
        {
            //add Firebase Database stuff
            ...
            myRef.child("Games").child("Important Info Before Checking Database on BattleActivity").child("After done everything, host claims p1 flinch is").setValue(activePokemon1.flinch);
            myRef.child("Games").child("Important Info Before Checking Database on BattleActivity").child("After done everything, host claims p2 flinch is").setValue(activePokemon2.flinch);
            myRef.child("Games").child("Important Info Before Checking Database on BattleActivity").child("After done everything, host claims p1 fainted is").setValue(activePokemon1.fainted);
            myRef.child("Games").child("Important Info Before Checking Database on BattleActivity").child("After done everything, host claims p2 fainted is").setValue(activePokemon2.fainted);
            myRef.child("Games").child("Important Info Before Checking Database on BattleActivity").child("After done everything, host claims p1 damage is").setValue(p1Damage);
            myRef.child("Games").child("Important Info Before Checking Database on BattleActivity").child("After done everything, host claims p2 damage is").setValue(p2Damage);
            myRef.child("Games").child("Important Info Before Checking Database on BattleActivity").child("After done everything, host claims p1 crit is").setValue(p1Critt);
            myRef.child("Games").child("Important Info Before Checking Database on BattleActivity").child("After done everything, host claims p2 crit is").setValue(p2Critt);
            myRef.child("Games").child("Important Info Before Checking Database on BattleActivity").child("After done everything, host claims p1 hit is").setValue(activePokemon1.hit);
            myRef.child("Games").child("Important Info Before Checking Database on BattleActivity").child("After done everything, host claims p2 hit is").setValue(activePokemon2.hit);
            myRef.child("Games").child("Important Info Before Checking Database on BattleActivity").child("After done everything, host claims p1 fail is").setValue(usedMove1.fail);
            myRef.child("Games").child("Important Info Before Checking Database on BattleActivity").child("After done everything, host claims p2 fail is").setValue(usedMove2.fail);
            myRef.child("Games").child("Important Info Before Checking Database on BattleActivity").child("After done everything, host claims p1 move is").setValue(usedMove1.name);
            myRef.child("Games").child("Important Info Before Checking Database on BattleActivity").child("After done everything, host claims p2 move is").setValue(usedMove2.name);
            myRef.child("Games").child("Important Info Before Checking Database on BattleActivity").child("After done everything, host claims p1 switch is").setValue(p1Switch);
            myRef.child("Games").child("Important Info Before Checking Database on BattleActivity").child("After done everything, host claims p2 switch is").setValue(p2Switch);
            myRef.child("Games").child("Important Info Before Checking Database on BattleActivity").child("After done everything, host claims p1 hp is").setValue(activePokemon1.hp);
            myRef.child("Games").child("Important Info Before Checking Database on BattleActivity").child("After done everything, host claims p2 hp is").setValue(activePokemon2.hp);
            myRef.child("Games").child("Important Info Before Checking Database on BattleActivity").child("After done everything, host claims start1 is").setValue(start1);
            myRef.child("Games").child("Important Info Before Checking Database on BattleActivity").child("After done everything, host claims start2 is").setValue(start2);
            myRef.child("Games").child("Important Info Before Checking Database on BattleActivity").child("After done everything, host claims absoluteStart1 is").setValue(absoluteStart1);
            myRef.child("Games").child("Important Info Before Checking Database on BattleActivity").child("After done everything, host claims absoluteStart2 is").setValue(absoluteStart2);
            myRef.child("Games").child("Important Info Before Checking Database on BattleActivity").child("After done everything, host claims healthAfterHit1 is").setValue(healthAfterHit1);
            myRef.child("Games").child("Important Info Before Checking Database on BattleActivity").child("After done everything, host claims healthAfterHit2 is").setValue(healthAfterHit2);
            myRef.child("Games").child("Important Info Before Checking Database on BattleActivity").child("After done everything, host claims healthAfterHealing1 is").setValue(healthAfterHealing1);
            myRef.child("Games").child("Important Info Before Checking Database on BattleActivity").child("After done everything, host claims healthAfterHealing2 is").setValue(healthAfterHealing2);
        }
    }
    ...
    void setLoadingGifForP2()
    {
        ImageView img2= (ImageView) findViewById(R.id.gifp2);
        img2.setImageResource(R.drawable.loadinggif);
    }
    ...
    void tellDatabaseMoveOrSwitch()
    {
        ...
        //If p1 switched, set switch on the database to p1Pokemon[i].orderNum, so we know what they switched to.
        //If they didnt switch, set switch to false so we know they are using a move and change Used Move on database
        // to the number of the used move

        if(p1Switch) {
            myRef.child("Games").child(Integer.toString(gameRoomNumGlobal)).child("Users").child(nameKeyOurs).child("Switch").setValue(Integer.toString(activePokemon1.orderNum));
        }
        else
        {
            myRef.child("Games").child(Integer.toString(gameRoomNumGlobal)).child("Users").child(nameKeyOurs).child("Switch").setValue("false");
            myRef.child("Games").child(Integer.toString(gameRoomNumGlobal)).child("Users").child(nameKeyOurs).child("Used Move").setValue(usedMove1.number);
        }

        Intent intent = new Intent(this, WaitForMoveOrSwitch.class);
